...
def buyerProfile(request):
    ...
    check = Checkout.objects.filter(buyer=buyer)
    check=check[::-1]
    return render(request,"buyerProfile.html",{'User':buyer, 'Wishlist':wishlist, 'Check':check})
...
def forgetPassword(request):
    if request.method=="POST":
        ...
        user.save()
        subject = "OTP to reset password for MyShop"
        message = f'Hi ! {user.name}, Your OTP to reset password for MyShop Account is: {user.otp}'
        email_from = "testpython180@gmail.com"
        recipient_list = [user.email, ]
        send_mail( subject, message, email_from, recipient_list )
        messages.success(request, f"OTP is Sent on {user.email}")
